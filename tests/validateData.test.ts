...
import { validateData, type Diagnostics } from "../src/lib/validateData";
...
describe("validateData", () => {
  it("全項目正常 → 空配列", async () => {
    ...
    const result = await validateData(getConn(), ["q1", "q3_1", "q3_2"], layout);
  ...
  it("SA 未定義コード → unknownCode error", async () => {
    ...
    const result = await validateData(getConn(), ["q1"], layout);
  ...
  it("SA カラム欠損 → dropped warn", async () => {
    ...
    const result = await validateData(getConn(), ["other"], layout);
    expect(result).toHaveLength(1);
  ...
  it("MA 不正値 (0/1以外) → invalidMAValue error", async () => {
    ...
    const result = await validateData(getConn(), ["q3_1", "q3_2"], layout);
    expect(result).toHaveLength(1);
    expect(result[0]).toMatchObject({
      type: "invalidMAValue",
      severity: "error",
  ...
  it("NA 非数値 → nonNumeric error", async () => {
    ...
    const result = await validateData(getConn(), ["age"], layout);
    expect(result).toHaveLength(1);
    expect(result[0]).toMatchObject({
      type: "nonNumeric",
      severity: "error",
  ...
  it("WEIGHT 非数値 → nonNumeric error", async () => {
    ...
    const result = await validateData(getConn(), ["w"], layout);
    expect(result).toHaveLength(1);
    expect(result[0]).toMatchObject({
      type: "nonNumeric",
      severity: "error",
  ...
  it("複数問題の混在 → 全件返却", async () => {
    ...
    const result = await validateData(getConn(), ["q1", "q3_1"], layout);
