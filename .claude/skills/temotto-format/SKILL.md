...
正式なスキーマ定義: `.claude/skills/temotto-format/layout.schema.json`（JSON Schema Draft 2020-12）

エディタで `$schema` を指定すると自動補完・バリデーションが有効になる:
```json
{
  "$schema": "./.claude/skills/temotto-format/layout.schema.json"
}
```
...
- レイアウトJSON Schema: `.claude/skills/temotto-format/layout.schema.json`
